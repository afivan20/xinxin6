...
async def fetch_lingo_data(begin: str, end: str) -> dict:
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            if data['code'] != 200:
                logger.warning(f"Не пришли данные от LingoAce. Ответ и код:<{data['message']} {data['code']}> Пробуем ещё.")
        ...
        try:
            async with session.get(url, ssl=False, headers=headers) as resp:
                ...
                if resp.status == 200:
                    ...
                    logger.warning(f"Не пришли данные от QKid. Ответ и код:<{data['message']} {data['code']} {resp.status}> Пробуем ещё.")
                    TEMP['token_QK'] = None
